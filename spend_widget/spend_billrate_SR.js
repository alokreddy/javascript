...
d3.json('spend.json').then(function (spend) {
    console.log(spend.data.spendbydaterange);
    ...
    moveChart
        ...
        .xUnits(d3.timeWeeks)
        .y(d3.scaleSqrt().domain([0, 1000, 10000]).range([0, 30]))
        .elasticY(true)
    ...
    locationChart.xAxis().tickFormat(d3.format('.2s')).ticks(5);
    locationChart.yAxis().tickFormat(d3.format('.2s')).ticks(5);
    //location bubble chart ends
    ...
    supplierChart.xAxis().tickFormat(d3.format('.2s')).ticks(7);
    supplierChart.yAxis().tickFormat(d3.format('.2s')).ticks(7);
